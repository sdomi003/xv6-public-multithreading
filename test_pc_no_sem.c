...
void test_producer_consumer_init() {
  ...
  if(sem_init(sem_cons,0,1) < 0) {
	  printf(0,"UNUSED semaphore cons initialize: FAIL\n");
  }
  else {
	  printf(0,"UNUSED semaphore cons initialize: PASS\n");
  }

  if(sem_init(sem_prod,0,1) < 0) {
	  printf(0,"UNUSED test semaphore prod initialize: FAIL\n");
  }
  else {
	  printf(0,"UNUSED test semaphore prod initialize: PASS\n");
  }
...
{
	printf(0, "STARTING PRODUCER-CONSUMER TEST WITHOUT SEMAPHORE\n");	
